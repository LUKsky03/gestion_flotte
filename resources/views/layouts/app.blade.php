...
<body class="bg-body">
    ...
    <div id="app"> 
        <nav class="navbar sticky-top navbar-expand-md navbar-light bg-navb   shadow-sm">
            <div class="container">
                @if(\Illuminate\Support\Facades\Auth::user())
                <a class="navbar-brand text-nav" href="{{ url('/home') }}">
                    Home
                </a>
                @endif
                <button class="navbar-toggler border-white text-g" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="{{ __('Toggle navigation') }}">
                            ...
                            <ul class="navbar-nav me-auto align-items-center ">
                                ...
                                <li class="nav-item dropdown d-none d-md-flex">
                                    @if(\Illuminate\Support\Facades\Auth::user()->hasRole(['admin', 'responsable auto']))
                                            <a href="#" class="mx-2 nav-link dropdown-toggle text-nav " role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
                                                Pages
                                            </a>
                                    @endif
                                ...
                                @if(\Illuminate\Support\Facades\Auth::user()->hasRole(['responsable fournisseurs']))
                                    <a href="{{url('/voitures-fournisseur')}}" class="dropdown-item color-nav">Gestion véhicules fournisseur</a>
                                @endif

                                @if(\Illuminate\Support\Facades\Auth::user()->hasRole(['admin', 'responsable agence']))
                                    <li class="nav-item">
                                        <a href="{{url('/agences')}}" class="mx-2 text-nav nav-link">Gestionnaire d'agences</a>
                                    </li>
                                @endif
                                @if(\Illuminate\Support\Facades\Auth::user()->hasRole(['admin', 'chef agence']))
                                    <li class="nav-item">
                                        <a href="{{url('/chef-agence')}}" class="mx-2 text-nav nav-link">Gestion agence</a>
                                    </li>
                                @endif
                                @if(\Illuminate\Support\Facades\Auth::user()->hasRole(['admin', 'secretaire']))
                                    <li class="nav-item">
                                        <a href="{{url('/locations')}}" class="mx-2 text-nav nav-link">Locations</a>
                                    </li>
                                @endif
                                @if(\Illuminate\Support\Facades\Auth::user()->hasRole(['admin', 'responsable fournisseurs']))
                                    <li class="nav-item">
                                        <a href="{{url('/fournisseurs')}}" class="mx-2 text-nav nav-link">Fournisseurs</a>
                                    </li>
                                @endif
                                @if(\Illuminate\Support\Facades\Auth::user()->hasRole(['admin','RH']))
                                    <li class="nav-item">
                                        <a href="{{url('/users')}}" class="mx-2 text-nav nav-link">Utilisateurs</a>
                                    </li>
                                @endif
                                @if(\Illuminate\Support\Facades\Auth::user()->hasRole(['admin','responsable commandes']))
                                    <li class="nav-item">
                                        <a href="{{url('/commandes')}}" class="mx-2 text-nav nav-link">Commandes</a>
                                    </li>
                                @endif
                            </ul>
                    @endif
